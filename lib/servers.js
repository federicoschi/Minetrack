...
class ServerRegistration {
  ...
  getUpdate (timestamp, resp, err, version) {
    ...
    if (resp) {
      ...
      if (config.logToDatabase) {
        ...
        update.historicalData = this.getHistoricalPlayerCounts()
      }
    } else if (err) {
  ...
  getPingHistory () {
    if (this._pingHistory.length > 0) {
      ...
      if (config.logToDatabase) {
        payload.historicalData = this.getHistoricalPlayerCounts()
      }
  ...
  getHistoricalPlayerCounts () {
    ...
    const bestMatches = {
      oneDayAgo: { diff: Infinity, playerCount: null },
      twoDaysAgo: { diff: Infinity, playerCount: null },
      threeDaysAgo: { diff: Infinity, playerCount: null }
    }

    for (let i = 0; i < this.graphData.length; i++) {
      const timestamp = this._app.timeTracker.getGraphPointAt(i) * 1000
      const playerCount = this.graphData[i]

      if (playerCount !== null && playerCount !== undefined) {
        const oneDayDiff = Math.abs(timestamp - oneDayAgo)
        if (oneDayDiff <= 60 * 1000 && oneDayDiff < bestMatches.oneDayAgo.diff) {
          bestMatches.oneDayAgo = { diff: oneDayDiff, playerCount }
        }

        const twoDaysDiff = Math.abs(timestamp - twoDaysAgo)
        if (twoDaysDiff <= 60 * 1000 && twoDaysDiff < bestMatches.twoDaysAgo.diff) {
          bestMatches.twoDaysAgo = { diff: twoDaysDiff, playerCount }
        }

        const threeDaysDiff = Math.abs(timestamp - threeDaysAgo)
        if (threeDaysDiff <= 60 * 1000 && threeDaysDiff < bestMatches.threeDaysAgo.diff) {
          bestMatches.threeDaysAgo = { diff: threeDaysDiff, playerCount }
        }
      }
    }

    if (bestMatches.oneDayAgo.playerCount !== null) {
      result.oneDayAgo = bestMatches.oneDayAgo.playerCount
    }
    if (bestMatches.twoDaysAgo.playerCount !== null) {
      result.twoDaysAgo = bestMatches.twoDaysAgo.playerCount
    }
    if (bestMatches.threeDaysAgo.playerCount !== null) {
      result.threeDaysAgo = bestMatches.threeDaysAgo.playerCount
    }
